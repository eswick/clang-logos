...
void CodeGenFunction::GenerateLogosMethodHook(const ObjCMethodDecl *OMD, ObjCHookDecl *Hook) {
    ...
    StartFunction(OMD, OMD->getReturnType(), Fn, FI, args, OMD->getLocStart());
...
void CodeGenFunction::GenerateObjCGetter(ObjCHookDecl *Hook,
    ...
    StartFunction(OMD, OMD->getReturnType(), Fn, FI, args, PID->getLocStart());
    ...
    RValue RV = RValue::get(Builder.CreateBitCast(
                    ret, 
                    getTypes().ConvertType(PID->getPropertyDecl()->getType())));

    EmitReturnOfRValue(RV, PID->getPropertyDecl()->getType());
...
void CodeGenFunction::GenerateObjCSetter(ObjCHookDecl *Hook,
  ...
  llvm::Value *objc_setAssociatedObjectArgs[] {
    Builder.CreateBitCast(LoadObjCSelf(), Int8PtrTy),
    llvm::ConstantExpr::getBitCast(Key, Int8PtrTy),
    Builder.CreateBitCast(obj, Int8PtrTy),
    Builder.getInt32(CalculateAssociationPolicy(PID->getPropertyDecl()))
  };
